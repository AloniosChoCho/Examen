                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="card-title card-title-with-button mb-4">
                                        ...
                                        <a class="btn btn-primary" href="{{ route('view.create') }}"><i class="fas fa-plus"></i> Nuevo</a>
                                    </div>
                                    @if (session('updated'))
                                    ...
                                    @endif
                                    @if (!count($view)){{-- contador de mi lista --}}
                                            ...
                                            @foreach ($view as $view)
                                              <tr>
                                                <td style="inline-size: 120px;" class="text-center">
                                                  <a href="{{route('view.edit', ['view' => $view->id]) }}" class="btn btn-sm btn-primary" title="Editar">
                                                    <i class="far fa-edit"></i>
                                                  </a>

                                                  <a href="javascript:;"
                                                    onclick="document.getElementById('form-update-state{{$view->id}}').submit()"
                                                    class="btn btn-sm btn-{{ $view->is_active ? 'success' : 'danger'}}"
                                                    title="{{ $view->is_active ? 'Desactivar' : 'Activar'}} ">
                                                    <i class="far fa-{{ $view->is_active ? 'check' : 'times'}}-circle"></i>
                                                  </a>
                                                  <form id="form-update-state{{$view->id}}" action="{{ route('view.update-is-active', ['view'=> $view->id ]) }}" method="post" class="d-none">
                                                    @csrf
                                                    <input type="hidden" name="state" value="{{$view->is_active}}">
                                                    <input type="hidden" name="idFaq" value="{{$view->id}}">
                                                  </form>
                                                    <form action="{{ route('view.destroy', ['view' => $view->id]) }}" method="POST" class="d-inline eliminar">
                                                    @csrf
                                                    @method('delete')
                                               ...
                                               <td>
                                                  <img src="{{ asset('storage/view/'.$view->imagen) }}"
                                                  onclick="openModalImage('{{$view->imagen}}')" style="block-size: 50px;border-radius: 10px;"
                                                  alt="Imagen de {{ $view->imagen }}">
                                                </td>
                                              </tr>
                                            @endforeach
                                          </tbody>
    ...
    <script>
      function closeModalImage() {
        ...
        function openModalImage(nameImage) {
          ...
          $imageFullSize.src = `${END_POINT}storage/view/${nameImage}`
        }
    </script>
